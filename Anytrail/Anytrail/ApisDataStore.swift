...
class ApisDataStore {
    ...
    func getTrailsWithCompletion(completion: (success: Bool) -> ()) {
        MashapeAPIClient.getTrails { (json, error) in
            self.mashapeData.removeAll()
            guard let json = json else {
                print("error: no data recieved from mashape API Client \(error)")
                completion(success: false)
                return
            }
            for object in json {
